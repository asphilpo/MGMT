...
def log_airtable_update_results(successful_files, failed_files_with_data, updated_files, log_dir="logs"):
    ...
    log_file_path = os.path.join(log_dir, f"airtable_update_log_{timestamp}.csv")
    print(log_file_path)

    with open(log_file_path, mode="w", newline="", encoding="utf-8") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["File Name", "Status", "Failure Payload (if any)"])

        for file in successful_files:
            writer.writerow([file, "Success", ""])

        for file in updated_files:
            writer.writerow([file, "Updated", ""])

        for file, payload in failed_files_with_data:
            payload_str = json.dumps(payload)
            writer.writerow([file, "Failed", payload_str])
